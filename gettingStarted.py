...
def transformData(trainDF, testDF):
    ...
    #Only the numerical attributes that didnt require pre-processing
    ...
    #The VP and I attributes found using correlation between numerical attributes and SalePrice attribute
    # vipPredictors = ['OverallQual', 'GrLivArea', 'YearBuilt', 'YearRemodAdd', 'TotalBsmtSF', '1stFlrSF', 'FullBath', 'TotRmsAbvGrd','GarageCars']
    ...
    vipPredictors = ['OverallQual', 'GrLivArea', 'YearBuilt', 'YearRemodAdd', 
                      ...
                      'MasVnrArea', 'BsmtQual', 'BsmtExposure', 'BsmtFinType1', 'BsmtFinType2', 'KitchenQual',
    ...
    testDF.loc[:, 'BsmtFullBath'] = testDF.loc[:, 'BsmtFullBath'].fillna(trainDF.loc[:, 'BsmtFullBath'].mode().iloc[0])
    ...
    #LotFrontage vlaues: numeric + missing values
    #Preprocessing method: missing values = mode(trainDf)
   ...
   # Values: NaN, 'AsbShng', 'AsphShn', 'Brk Cmn', 'BrkFace', 'CBlock', 'CmentBd', 'HdBoard', 'ImStucc', 
   #                'MetalSd', 'Other', 'Plywood', 'PreCast', 'Stone', 'Stucco', 'VinylSd', 'Wd Sdng', 'Wd Shng'
    ...
    #MasVnrArea vlaues: numeric + missing values
    #Preprocessing method: missing values = mode(trainDf)
    ...
    #BsmtFinSF1 values: numeric + missing values
    #Preprocessing method: missing values = mode(trainDf)
    ...
    testDF.loc[:, 'BsmtFinType1'] = testDF.loc[:, 'BsmtFinType1'].fillna(trainDF.loc[:, 'BsmtFinType1'].mode().iloc[0])
    #----------------------------------------------------------------------------------------------------
    ...
    trainDF.loc[:, 'BsmtFinType2'] = trainDF.loc[:, 'BsmtFinType2'].fillna(trainDF.loc[:, 'BsmtFinType1'].mode().iloc[0])
    ...
    testDF.loc[:, 'BsmtFinType2'] = testDF.loc[:, 'BsmtFinType2'].fillna(trainDF.loc[:, 'BsmtFinType2'].mode().iloc[0])
    #----------------------------------------------------------------------------------------------------
    ...
    testDF.loc[:, 'GarageType'] = testDF.loc[:, 'GarageType'].fillna(trainDF.loc[:, 'GarageType'].mode().iloc[0])
    #---------------------------------------------------------------------------------------------------- 
    
    #GarageYrBlt Values: numeric values + missing values 
    #Preprocessing Method: missing = Mode(trainDf)
...
def findCorr(df):
    ...
    corr = corr.map(lambda v: 'VP' if v > 0.7 else ("I" if v > 0.5 else ("R" if v > 0.3 else "N")))
